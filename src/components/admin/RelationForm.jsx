...
export default function RelationForm({ relation, onSave, onCancel }) {
  ...
  useEffect(() => {
    if (relation) {
      setFormData({
        ...
        isActive: relation.isActive !== undefined ? relation.isActive : true,
      });
      setExistingImageUrl(relation.imageUrl || null);
      setImagePreview(relation.imageUrl || null);
      setImage(null);
    } else {
    ...
    setTimeout(() => {
      initialSnapshotRef.current = JSON.stringify({
        ...
        existingImageUrl: relation?.imageUrl || null,
        imageSelected: false,
        imagePreview: !!relation?.imageUrl,
      });
    }, 0);
  }, [relation]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = relation ? `${API}/relations/${relation.id}` : `${API}/relations`;
      ...
      if (res.ok) {
        toast.success(relation ? "Relation updated" : "Relation created");
        onSave();
        setFormData({ name: "", slug: "", description: "", isActive: true });
        setImage(null);
        setImagePreview(null);
        ...
        toast.error(data.error || data.message || "Failed to save relation");
      }
    } catch (error) {
      toast.error(error.message || "Failed to save relation");
    } finally {
      setLoading(false);
    ...
    <div className="bg-white rounded-xl shadow-md p-6 mb-6 border border-gray-200">
      <div className="flex items-start justify-between gap-4 mb-6">
        <h2 className="text-xl font-bold text-gray-900">
          {relation ? "Edit Relation" : "Add New Relation"}
        </h2>
        <div className="flex gap-2 shrink-0">
          ...
          >
            ...
            {relation ? "Update" : "Save"}
          </button>
        </div>
      </div>
      <form ref={formRef} onSubmit={handleSubmit} onKeyDown={handleKeyDown} className="space-y-4">
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">Relation Name *</label>
          <input
            type="text"
            value={formData.name}
            onChange={(e) => {
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">Relation Image</label>
